...
const PurchaseOrderAdd = () => {
  ...
  const handleAssignBarcode = useCallback(async (row) => {
    ...
    try {
      ...
      console.error('[Purchase order add] Failed to assign barcode', err);
  ...
  const findExactProductForScan = useCallback(async (query) => {
    ...
    try {
      ...
      console.warn('[Purchase order add] Barcode lookup failed', err);
      return null;
    }
  }, [addProductResults]);
  ...
  const handleProductSearchKeyDown = useCallback(
    async (e) => {
      ...
      e.preventDefault();
      // Prefer the ref (updated synchronously in onChange) — scanners type + Enter
      // faster than React state flushes.
      const q = String(
        addProductQueryRef.current ||
                ...
                <div className="po-form-section">
                  ...
                  <div className="row g-3">
                    ...
                    <div className="col-md-6 col-lg-4">
                      <label className="form-label" htmlFor="po-add-created-on-date">
                        Created on
                      </label>
                      <div className="po-datetime">
                        <input
                          id="po-add-created-on-date"
                          type="date"
