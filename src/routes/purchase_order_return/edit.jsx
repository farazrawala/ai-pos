...
const PurchaseOrderReturnEdit = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('[Purchase order return edit] Failed to load payment accounts', err);
        if (!cancelled) {
          setAccounts([]);
  ...
  useEffect(() => {
    if (id) dispatch(fetchPurchaseOrderReturnById(id));
    return () => {
      dispatch(clearCurrentPurchaseOrderReturn());
    ...
    async (product) => {
      ...
      if (productPickerWholesalePrice(product) === null) {
        try {
          ...
          console.warn('[Purchase order return edit] Could not load product wholesale price', err);
        }
      }
  ...
  if (fetchStatus === 'loading') {
    return (
      <div className="container-fluid py-4">
        <p className="text-muted mb-0">Loading purchase order return…</p>
      </div>
    );
  }
    ...
    return (
      <div className="container-fluid py-4">
        <div className="alert alert-danger" role="alert">
          {fetchError || 'Failed to load purchase order return.'}
        </div>
        <button type="button" className="btn btn-sm btn-outline-secondary" onClick={() => navigate('/purchase-order-returns')}>
          Back to list
        </button>
      </div>
    );
...
export default PurchaseOrderReturnEdit;
