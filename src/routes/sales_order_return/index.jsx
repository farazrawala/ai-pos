...
import { fetchAllSalesReturnsForExportRequest } from '../../features/salesReturns/salesReturnsAPI.js';
import {
  SALES_RETURN_EXPORT_COLUMNS,
  mapSalesReturnsToExportRows,
} from '../../features/salesReturns/salesReturnExportMapper.js';
...
const SalesReturns = () => {
  ...
  const handleExport = async (format) => {
    setExporting(true);
    try {
      const records = await fetchAllSalesReturnsForExportRequest(buildExportParams());
      if (!records.length) {
        toast.info('No sales returns to export.');
        return;
      }
      const mapped = mapSalesReturnsToExportRows(records);
      const stamp = moment().format('YYYY-MM-DD-HHmm');
      const filename = `sales-returns-${stamp}`;
      if (format === 'csv') {
        exportRowsToCsv({ columns: SALES_RETURN_EXPORT_COLUMNS, rows: mapped, filename });
      } else if (format === 'excel') {
        exportRowsToExcel({
          columns: SALES_RETURN_EXPORT_COLUMNS,
          rows: mapped,
          filename,
          sheetTitle: 'Sales returns',
        });
      } else if (format === 'pdf') {
        await exportRowsToPdf({
          ...
          rows: mapped,
          filename,
          title: 'Sales returns (with line items)',
        });
      }
      toast.success(
      ...
      console.error('[Sales returns] export failed', err);
                ...
                <div className="col-lg-8 col-md-7">
                  <div className="d-flex flex-wrap justify-content-md-end align-items-center gap-2 mt-2 mt-md-0">
                    ...
                    <button
                      ...
                      aria-controls="sales-returns-filter-panel"
