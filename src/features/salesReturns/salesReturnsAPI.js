...
function assertSalesReturnJsonSuccess(result) {
...
const ENDPOINT_PATH = 'sales_order_return/get-sales-order-return-by-order-item';
const LIST_ALL_ACTIVE_PATH = 'sales_return/get-all-active';
const GET_BY_RETURN_NO_PATH = 'sales_return/get-sales-return-by-return-no';

/** Appended on GET list/detail for this route so `customer_id` is populated (e.g. Mongoose). */
const SALES_RETURN_GET_POPULATE = 'customer_id';
/** @deprecated use SALES_RETURN_GET_POPULATE */
const SALES_ORDER_GET_POPULATE = SALES_RETURN_GET_POPULATE;

/**
 * Query / JSON body keys to try (in order). Backends vary: snake_case, camelCase, generic `id`.
 * Override order via `.env`: `VITE_SALES_RETURN_ITEM_PARAM` = single key to use only.
 */
...
const getParamKeysToTry = () => {
  const fromEnv =
    typeof import.meta !== 'undefined' && import.meta.env?.VITE_SALES_RETURN_ITEM_PARAM
      ? String(import.meta.env.VITE_SALES_RETURN_ITEM_PARAM).trim()
      : '';
  if (fromEnv) return [fromEnv];
  return DEFAULT_PARAM_KEYS;
};

const logSalesReturnModuleError = (operation, details) => {
  console.error(`[Sales return module] ${operation}`, details);
};
 ...
 */
export const normalizeSalesReturnByItemPayload = (json) => {
...
/**
 * Primary query key for the SPA URL (`/sales-returns?...`). Defaults to first backend key we try.
 */
export const ORDER_ITEM_QUERY_KEY = getParamKeysToTry()[0];
...
async function fetchJsonOnce(method, url, init = {}) {
  ...
  try {
    ...
    logSalesReturnModuleError('fetch network error', { url, method, error: err });
    throw err;
  }
...
/**
 * GET/POST `sales_order_return/get-sales-order-return-by-order-item` with several param/body key names.
 *
 * @param {string} orderItemId
 * @returns {Promise<object|Array|null>}
 */
export const fetchSalesReturnByOrderItemRequest = async (orderItemId) => {
  ...
  for (const key of paramKeys) {
    ...
    query.set(key, id);
    query.set('populate', SALES_RETURN_GET_POPULATE);
    const url = `${baseUrl}?${query.toString()}`;
    try {
      const result = await fetchJsonOnce('GET', url, { headers: getJsonReadHeaders() });
      const normalized = normalizeSalesReturnByItemPayload(result);
      return normalized;
    } catch (e) {
      lastErr = e;
      const st = e?.status;
      if (st === 400 || st === 404 || st === 422) {
        if (typeof import.meta !== 'undefined' && import.meta.env?.DEV) {
          console.debug('[Sales return module] GET try next param', { key, status: st });
        }
        continue;
      }
      throw e;
  ...
  for (const key of paramKeys) {
    ...
    try {
      ...
      if (st === 400 || st === 404 || st === 405 || st === 422) {
        if (typeof import.meta !== 'undefined' && import.meta.env?.DEV) {
          console.debug('[Sales return module] POST try next param', { key, status: st });
        }
        continue;
      }
      throw e;
  ...
  logSalesReturnModuleError('fetchSalesReturnByOrderItemRequest exhausted', {
    orderItemId: id,
    lastError: lastErr?.message,
  });
  throw lastErr || new Error('Could not load sales return for this item id');
};
 ...
 */
export function normalizeSalesReturnsListResponse(result, params = {}) {
  ...
  else if (result && typeof result === 'object') rows = normalizeSalesReturnByItemPayload(result);
...
/**
 * Paginated list: `GET sales_return/get-all-active`
 * with optional `populate=customer_id`, `skip`, `limit`, `search`, `sortBy`, `sortOrder`.
 */
export async function fetchSalesReturnsListRequest(params = {}) {
  const queryParams = new URLSearchParams();
  const page = Math.max(1, Number(params.page) || 1);
  const limit = Math.max(1, Number(params.limit) || 10);
  const skip = (page - 1) * limit;
  queryParams.append('populate', SALES_RETURN_GET_POPULATE);
  ...
  try {
    ...
    logSalesReturnModuleError('fetchSalesReturnsListRequest network error', { url, err });
    throw err;
  }

  if (!response.ok) {
    const message = await getErrorMessageFromResponse(response);
    logSalesReturnModuleError('fetchSalesReturnsListRequest failed', {
 ...
 */
export function unwrapSalesReturnRecord(result) {
...
/**
 * GET `sales_return/get-sales-return-by-return-no/:id?populate=customer_id`
 * — `id` is sales return `_id` or `sales_return_no`.
 */
export async function fetchSalesReturnByIdRequest(salesReturnId) {
  const id = String(salesReturnId ?? '').trim();
  if (!id) throw new Error('Sales return id is required');
  const qs = new URLSearchParams({ populate: SALES_RETURN_GET_POPULATE }).toString();
  const url = `${BASE_URL}${GET_BY_RETURN_NO_PATH}/${encodeURIComponent(id)}?${qs}`;
  let response;
  try {
    response = await fetch(url, { method: 'GET', headers: getJsonReadHeaders() });
  } catch (err) {
    logSalesReturnModuleError('fetchSalesReturnByIdRequest network error', { url, err });
    throw err;
  }
  if (!response.ok) {
    const message = await getErrorMessageFromResponse(response);
    logSalesReturnModuleError('fetchSalesReturnByIdRequest failed', {
      id,
      status: response.status,
      message,
    });
  ...
  assertSalesReturnJsonSuccess(json);
  ...
  const unwrapped = unwrapSalesReturnRecord(json);
...
/**
 * POST `sales_return/sales_return_create` — multipart form fields:
 * `customer_id`, `description`, `ref_no`, `discount`, `shipment`, `account_id`,
 ...
 *
 * UI may send `customer_id`, `sales_order_no`, `notes`, and `items[]` with per-line
 * `shipping_per_unit` / `total_shipping`; those are mapped to indexed form fields.
 */
export async function createSalesReturnRequest(payload = {}) {
  ...
  const refNo = body.ref_no ?? body.sales_order_no;
  ...
  const url = `${BASE_URL}sales_return/sales_return_create`;
  let response;
  try {
    ...
    logSalesReturnModuleError('createSalesReturnRequest network error', { url, err });
    throw err;
  }
  if (!response.ok) {
    const message = await getErrorMessageFromResponse(response);
    logSalesReturnModuleError('createSalesReturnRequest failed', {
      status: response.status,
      message,
    });
    throw new Error(message);
...
/**
 * PATCH `sales_order_return/sales_order_return_update/:id` — multipart form fields:
 * `name`, `email`, `phone`, `address`, `customer_id`, `description`, `ref_no`,
 * `product_id[n]`, `qty[n]`, `price[n]`, `warehouse_id[n]`, `warehouse_inventory_id[n]` (optional),
 ...
 */
export async function updateSalesReturnRequest(salesReturnId, payload = {}) {
  const id = String(salesReturnId ?? '').trim();
  if (!id) throw new Error('Sales return id is required');
  const body = payload && typeof payload === 'object' ? payload : {};
  const form = new FormData();
  ...
  const url = `${BASE_URL}sales_order_return/sales_order_return_update/${encodeURIComponent(id)}`;
  let response;
  try {
    response = await fetch(url, {
      method: 'PATCH',
    ...
    logSalesReturnModuleError('updateSalesReturnRequest network error', { url, err });
    throw err;
  }
  if (!response.ok) {
    const message = await getErrorMessageFromResponse(response);
    logSalesReturnModuleError('updateSalesReturnRequest failed', {
      id,
      status: response.status,
      message,
    });
...
/**
 * DELETE `sales_return/sales_return_delete/:salesReturnId`
 */
export async function deleteSalesReturnRequest(salesReturnId) {
  const id = String(salesReturnId ?? '').trim();
  if (!id) {
    throw new Error('Sales return id is required');
  }

  const url = `${BASE_URL}sales_return/sales_return_delete/${encodeURIComponent(id)}`;
  let response;
  try {
    response = await fetch(url, {
      method: 'DELETE',
    ...
    logSalesReturnModuleError('deleteSalesReturnRequest network error', { url, id, err });
    throw err;
  }

  if (!response.ok) {
    const message = await getErrorMessageFromResponse(response);
    logSalesReturnModuleError('deleteSalesReturnRequest failed', {
      id,
      status: response.status,
      message,
    });
