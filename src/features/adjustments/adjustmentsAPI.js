...
/** Body for POST /adjustment/create and PATCH /adjustment/update/:id */
...
/** PATCH /adjustment/update/:id */
export async function updateAdjustmentRequest(adjustmentId, adjustmentData = {}) {
  const token = getAuthToken();
  const id = String(adjustmentId ?? '').trim();
  if (!id) throw new Error('Missing adjustment id');

  const url = `${BASE_URL}adjustment/update/${encodeURIComponent(id)}`;
...
export async function createAdjustmentRequest(adjustmentData = {}) {
  const token = getAuthToken();
  const url = `${BASE_URL}adjustment/create`;
  const body = buildAdjustmentCreateBody(adjustmentData);
  ...
  try {
    ...
    logAdjustmentModuleError('createAdjustmentRequest network error', {
      url,
      payloadKeys: Object.keys(body),
      error: err,
    });
  ...
  if (!response.ok) {
    const details = await readResponseErrorDetails(response);
    logAdjustmentModuleError('createAdjustmentRequest failed', {
      url,
      payloadKeys: Object.keys(body),
      ...details,
    });
  ...
  try {
    ...
    logAdjustmentModuleError('createAdjustmentRequest invalid JSON on success', {
      url,
      error: parseErr,
    });
    return { success: true };
