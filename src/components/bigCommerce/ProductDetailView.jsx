...
export default function ProductDetailView({
  ...
  return (
    <div className={`bc-detail${variant === 'page' ? ' bc-detail--page' : ''}`}>
      {variant === 'page' ? (
        <header className="bc-product-page-head">
          <h1 className="bc-product-page-title">{name}</h1>
          <p className="bc-product-page-sub">
            {[brand.name !== '—' ? brand.name : '', category.name !== '—' ? category.name : '']
              .filter(Boolean)
      ...
      <div className="bc-detail-info">
        ...
        {showInlineActions && !hideMeToo ? (
          <div className="bc-product-page-actions">
            <ProductDetailActions {...actionProps} />
          </div>
        ) : null}
        ...
        {isVariable || variations.length > 0 ? (
          <div className="bc-detail-section">
            <div className="bc-detail-section-head">
              <h4>Variations</h4>
              {variations.length > 0 ? <span className="bc-pill">{variations.length}</span> : null}
            </div>
            {variations.length > 0 ? (
