...
export default function SyncBrandsModal({ open, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const brandsResult = await fetchBrandsRequest({ page: 1, limit: 5000 });
      const brandIds = (brandsResult?.data || []).map(brandIdFromRecord).filter(Boolean);

      if (brandIds.length === 0) {
        setSaveStatus('failed');
        setSaveError('No brands found to sync.');
        return;
      }

      await createBulkSyncBrandProcessRequest(selectedIntegrationId, brandIds);
      setSaveStatus('succeeded');
      onSaved?.();
      onClose?.();
    } catch (err) {
      setSaveStatus('failed');
      setSaveError(err?.message || 'Failed to queue brand sync processes');
    }
  };
  ...
  return (
    <>
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="syncBrandsModalLabel"
        aria-modal="true"
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="syncBrandsModalLabel">
                Sync Brands
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              <p className="text-sm text-muted mb-3">
                Select a store integration to push your POS brands to WooCommerce or another
                connected platform.
              </p>
              ...
              {integrationsStatus === 'succeeded' && integrations.length > 0 && (
                <div className="mb-0">
                  <label htmlFor="syncBrandsIntegration" className="form-label">
                    Integration <span className="text-danger">*</span>
                  </label>
                  <select
                    id="syncBrandsIntegration"
