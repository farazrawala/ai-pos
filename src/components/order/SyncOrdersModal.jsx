...
import { fetchOrdersRequest, pickOrderDocumentId } from '../../features/orders/ordersAPI.js';
...
export default function SyncOrdersModal({ open, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const ordersResult = await fetchOrdersRequest({ page: 1, limit: 5000 });
      const orderIds = [
        ...new Set(
          (ordersResult?.data || []).map((item) => pickOrderDocumentId(item)).filter(Boolean)
        ),
      ];

      if (orderIds.length === 0) {
        setSaveStatus('failed');
        setSaveError('No orders found to sync.');
        return;
      }

      await createBulkSyncOrderProcessRequest(selectedIntegrationId, orderIds);
      setSaveStatus('succeeded');
      onSaved?.();
      onClose?.();
    } catch (err) {
      setSaveStatus('failed');
      setSaveError(err?.message || 'Failed to queue order sync processes');
    }
  };
  ...
  return (
    <>
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="syncOrdersModalLabel"
        aria-modal="true"
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="syncOrdersModalLabel">
                Sync Orders
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              <p className="text-sm text-muted mb-3">
                Select a store integration to push your POS orders to WooCommerce or another
                connected platform.
              </p>
              ...
              {integrationsStatus === 'succeeded' && integrations.length > 0 && (
                <div className="mb-0">
                  <label htmlFor="syncOrdersIntegration" className="form-label">
                    Integration <span className="text-danger">*</span>
                  </label>
                  <select
                    id="syncOrdersIntegration"
