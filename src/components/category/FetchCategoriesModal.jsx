...
import { createSyncCategoryProcessRequest } from '../../features/process/processAPI.js';
...
export default function FetchCategoriesModal({ open, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      await createSyncCategoryProcessRequest(selectedIntegrationId);
      setSaveStatus('succeeded');
      onSaved?.();
      onClose?.();
    } catch (err) {
      setSaveStatus('failed');
      setSaveError(err?.message || 'Failed to queue category sync process');
    }
  };
  ...
  return (
    <>
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="fetchCategoriesModalLabel"
        aria-modal="true"
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="fetchCategoriesModalLabel">
                Fetch Categories
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              <p className="text-sm text-muted mb-3">
                Select a store integration to import categories from WooCommerce or another
                connected platform.
              </p>
              ...
              {integrationsStatus === 'succeeded' && integrations.length > 0 && (
                <div className="mb-0">
                  <label htmlFor="fetchCategoriesIntegration" className="form-label">
                    Integration <span className="text-danger">*</span>
                  </label>
                  <select
                    id="fetchCategoriesIntegration"
