...
const recordName = (record) => {
  ...
  if (typeof record === 'object') {
    return decodeHtml(record.name || record.category_name || '-');
  }
...
export default function ViewCategorySyncModal({ open, categoryId, categoryName, onClose }) {
  ...
  const loadSyncRecords = useCallback(() => {
    ...
    fetchSyncCategoriesRequest({
      ...
      populate: 'category_id,integration_id',
    })
      .then((result) => {
        if (cancelled) return;
      ...
      .catch((err) => {
        if (!cancelled) {
          ...
          setError(err?.message || 'Failed to load category sync records');
        }
      });
  ...
  useEffect(() => {
    if (!open || !categoryId) return undefined;
    setList([]);
  ...
  const handleToggleStatus = async (syncId, isCurrentlyActive) => {
    ...
    try {
      await updateSyncCategoryRequest(syncId, { status: newStatus });
      setList((prev) =>
        prev.map((item) =>
      ...
      console.error('[Sync category module] Failed to toggle sync status', { syncId, error: err });
    } finally {
  ...
  const title = categoryName ? decodeHtml(categoryName) : 'Category';
  ...
  return (
    <>
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="viewCategorySyncModalLabel"
        aria-modal="true"
      >
        <div className="modal-dialog modal-dialog-centered modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="viewCategorySyncModalLabel">
                Category Sync — {title}
              </h5>
            ...
            <div className="modal-body">
              ...
              {loadStatus === 'succeeded' && list.length === 0 && (
                <div className="alert alert-warning py-2 mb-0">
                  No sync records found for this category.
                </div>
              )}
                      ...
                      {list.map((item, index) => {
                        ...
                        return (
                          <tr key={rowId || index}>
                            ...
                            <td className="text-sm">{recordName(item.category_id)}</td>
                            <td className="text-sm">
