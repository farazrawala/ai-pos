...
export default function SyncCategoriesModal({ open, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const categoriesResult = await fetchCategoriesRequest({ page: 1, limit: 5000 });
      const categoryIds = (categoriesResult?.data || [])
        .map(categoryIdFromRecord)
        .filter(Boolean);

      if (categoryIds.length === 0) {
        setSaveStatus('failed');
        setSaveError('No categories found to sync.');
        return;
      }

      await createBulkSyncCategoryProcessRequest(selectedIntegrationId, categoryIds);
      setSaveStatus('succeeded');
      onSaved?.();
      onClose?.();
    } catch (err) {
      setSaveStatus('failed');
      setSaveError(err?.message || 'Failed to queue category sync processes');
    }
  };
  ...
  return (
    <>
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="syncCategoriesModalLabel"
        aria-modal="true"
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="syncCategoriesModalLabel">
                Sync Categories
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              <p className="text-sm text-muted mb-3">
                Select a store integration to push your POS categories to WooCommerce or another
                connected platform.
              </p>
              ...
              {integrationsStatus === 'succeeded' && integrations.length > 0 && (
                <div className="mb-0">
                  <label htmlFor="syncCategoriesIntegration" className="form-label">
                    Integration <span className="text-danger">*</span>
                  </label>
                  <select
                    id="syncCategoriesIntegration"
